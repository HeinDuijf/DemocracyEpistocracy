...
def combined_lineplot(
    ...
    for rel_mean_idx, rel_mean in enumerate(rel_means):
        ...
        for n_sources, linewidth in zip(n_sources_sorted, linewidths):
            ...
            y_values = []
            for r in restricted_list_sorted:
                vals = df_agg_n[
                    (df_agg_n["group_type"] == f"restricted_{r}")
            ...
            ax.axhline(expert_value, color=color, linestyle=":", linewidth=linewidth)
    ...
    style_labels = [str(n) for n in n_sources_sorted] + ["expert (delib)"]
    ax.legend(
        style_handles,
        style_labels,
        title=r"Sources (\#)",
    ...
    if filename is None:
        ...
        filename = f"figures/images/lineplot_combined_{n_sources_str}_{measure}"

...
def generate_expert_team(sources: Sources, heuristic_size: int | list, size: int):
...
def generate_diverse_team(sources: Sources, heuristic_size: int | list, size: int):
    if not isinstance(heuristic_size, list):
        heuristic_size = [heuristic_size]
...
if __name__ == "__main__":
    ...
    team = generate_restricted_team(
        sources, heuristic_size=5, size=9, qualifying_percentile=90
    )
    mid = time.time()

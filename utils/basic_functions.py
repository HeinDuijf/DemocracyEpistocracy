...
def calculate_competence(reliabilities: list[float] | NDArray[np.float64]) -> float:
    competence: float = 0
...
def calculate_competence_bayesian(
    reliabilities: list[float] | NDArray[np.float64],
) -> float:
    sources = np.array(range(len(reliabilities)))
    accuracy: float = 0.0
    for sources_positive in powerset(sources):
        p_sources_positive_given_positive = np.prod(
            [reliabilities[source] for source in sources_positive],
            dtype=float,
        ) * np.prod(
            [
        ...
        p_sources_positive_given_negative = np.prod(
            [1 - reliabilities[source] for source in sources_positive],
            dtype=float,
        ) * np.prod(
            [
        ...
        if p_sources_positive_given_positive > p_sources_positive_given_negative:
            accuracy += p_sources_positive_given_positive
    return accuracy
